...
The sidebar `视频` feature is a native Swift `yt-dlp` process wrapper in `VideoDownloadClient`; do not route video downloads through the Python media worker. The default executable path is `ToolSettings.homebrewYtDLPPath` (`/opt/homebrew/bin/yt-dlp`) and is stored alongside other tool paths. Keep `VideoDownloadPreset` labels, `VideoDownloadClient` argument generation, settings persistence in `AppSettings.videoDownload`, and `VideoDownloadClientTests` aligned when changing download choices. Normal user UI should show readable preset names such as `视频+音频，兼容 MP4`, not raw `yt-dlp`/FFmpeg arguments.

Video downloads are tracked in the same recent-job store as audio processing using `RecentJob.kind == .videoDownload`; older persisted records without a kind must continue to decode as `.audio`. Completed recent tasks should remain grouped by kind (`音声` and `视频下载`). Downloaded media files are user-visible outputs and should be written directly into the selected output directory, while internal logs and working data stay under `output/.otochef/<job-id>/`.
