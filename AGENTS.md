...
Output artifacts are user-selected through `VideoSettings.outputFiles`; keep the settings UI, validation, Swift job JSON, Python model parsing, and worker pipeline in sync. Audio tasks use `AppSettings.video`, while local video localization tasks use `AppSettings.localizedVideo`; keep the two output-setting groups independent. Supported outputs are video, source-language subtitles, Chinese subtitles, and bilingual subtitles, and at least one must be selected. For audio tasks, only require an image and FFmpeg when video is selected. For local video tasks, use the uploaded video as the visual source, do not require a still image, and use FFmpeg to extract audio for native WhisperKit before the Python worker renders selected outputs. Only require translation settings and API keys when Chinese, bilingual, or video output is selected. Use `VideoSettings.subtitleOutputMode` only for selected video output: `mkvSoftAss` creates `output.mkv` with ASS soft subtitles, while `mp4HardSubtitles` creates `output.mp4` with ASS burned in and requires an FFmpeg build with the `subtitles` filter. User-visible artifacts should be written directly into the selected output directory, defaulting to project-root `output/`; internal files such as `job.json`, `transcript.ja.json`, and `translation.zh.json` belong under the hidden working directory `output/.otochef/<job-id>/`. Treat translation output as untrusted text for subtitle formats: preserve visible text, but neutralize ASS override/control syntax before writing `.ass`.

## Video Downloads

The sidebar `视频下载` feature is a native Swift `yt-dlp` process wrapper in `VideoDownloadClient`; do not route video downloads through the Python media worker. The separate sidebar `视频` feature is for local video localization and follows the normal native-ASR plus Python-worker processing path. The default executable path is `ToolSettings.homebrewYtDLPPath` (`/opt/homebrew/bin/yt-dlp`) and is stored alongside other tool paths. Keep `VideoDownloadPreset` labels, `VideoDownloadClient` argument generation, settings persistence in `AppSettings.videoDownload`, and `VideoDownloadClientTests` aligned when changing download choices. Normal user UI should show readable preset names such as `视频+音频，兼容 MP4`, not raw `yt-dlp`/FFmpeg arguments.

Video downloads are tracked in the same recent-job store as audio and local video processing using `RecentJob.kind == .videoDownload`; older persisted records without a kind must continue to decode as `.audio`. Completed recent tasks should remain grouped by kind (`音声`, `视频`, and `视频下载`). Downloaded media files are user-visible outputs and should be written directly into the selected output directory, while internal logs and working data stay under `output/.otochef/<job-id>/`.
