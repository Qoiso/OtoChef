...
OtoChef 可以在设置页自动配置 Python worker、FFmpeg、yt-dlp 和 Deno。受管运行环境保存在已忽略的项目目录 `.otochef-runtime/` 下，不会修改系统 Python、Homebrew、Shell 配置或已有 Conda 环境。
...
正常使用应用时，请在设置页点击**一键配置环境**。如果项目受管环境已经完整，OtoChef 会直接检测并复用，不会重复安装。

开发者仍可通过命令行创建或更新旧的命名 Conda 环境：
